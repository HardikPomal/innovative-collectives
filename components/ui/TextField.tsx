...
const TextField = forwardRef<HTMLInputElement, TextFieldProps>(({
    ...
    return (
        <div className={className}>
            ...
            <div
                className={`flex items-center rounded-full px-4 py-2.5 sm:py-3 border transition-all ${
                    props.disabled
                        ? "bg-cream/60 border-navy/10 cursor-not-allowed opacity-80"
                        : error
                        ? "bg-white border-red-400 focus-within:ring-2 focus-within:ring-red-100"
                        : "bg-white border-navy/15 focus-within:border-gold focus-within:ring-2 focus-within:ring-gold/15"
                }`}
            >
                {icon && (
                ...
                <input
                    ...
                    type={resolvedType}
                    aria-invalid={!!error}
                    className={`flex-1 min-w-0 bg-transparent font-body text-sm text-navy placeholder:text-navy/40 outline-none ${props.disabled ? "cursor-not-allowed text-navy/60" : ""} ${inputClassName}`.trim()}
                    {...props}
                />
